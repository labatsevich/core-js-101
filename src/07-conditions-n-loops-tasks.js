...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (obj1.x1 > obj2.x2 || obj2.x1 > obj1.x2) return false;
  if (obj1.y2 < obj2.y1 || obj2.y2 < obj1.y1) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  const d = Math.abs((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
...
function isCreditCardNumber(ccn) {
  const sum = String(ccn).split('').reverse().map((num, i) => {
    if (i % 2 !== 0) {
...
function getDigitalRoot(num) {
  const sum = String(num).split('').reduce((prev, current) => +prev + +current);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const bracket = map[current];

    if (!bracket) stack.push(current);
